<template>
  <div class="min-h-full bg-[#f5f6fa] p-4 lg:p-8">
    <component :is="'style'">
      @import
    ...
    <!-- Page Header -->
    <div
      class="bg-white rounded-xl shadow-sm border border-slate-100 px-4 sm:px-6 py-4 mb-6"
    >
      <div class="flex items-center gap-3 sm:gap-4">
        <div class="w-1.5 h-10 rounded-full bg-[#3d4f7c] flex-shrink-0"></div>
        <div
          class="w-10 h-10 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center bg-[#3d4f7c]/10 text-[#3d4f7c] flex-shrink-0"
        >
          <svg
            width="20"
            height="20"
        ...
        <div class="min-w-0">
          <h1
            class="text-xl sm:text-3xl font-semibold text-slate-800 tracking-tight"
          >
            Service Requests
          </h1>
          <p class="text-xs sm:text-sm text-slate-500 mt-0.5 hidden sm:block">
            View, track, and manage all service requests across the barangay
          </p>
        </div>
      </div>
    </div>
    ...
    <!-- Filters -->
    <div
      class="bg-white rounded-xl border border-slate-100 p-3 sm:p-4 mb-5 shadow-sm"
    >
      <div class="flex flex-col sm:flex-row sm:items-center gap-2.5">
        ...
        <div class="flex items-center gap-2">
          ...
          >
            ...
            <option value="certificate_of_indigency">
              Certificate of Indigency
            </option>
            <option value="certificate_of_residency">
    ...
    >
      <!-- Header -->
      <div
        class="px-4 sm:px-6 py-4 bg-gradient-to-r from-[#3d4f7c] to-[#252b3b]"
      >
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="w-1.5 h-8 rounded-full bg-white/20"></div>
            <h2
              class="text-base sm:text-lg font-semibold text-white tracking-tight"
            >
      ...
      >
        <div class="relative w-11 h-11">
          <div
            class="absolute inset-0 border-[3px] border-slate-100 rounded-full"
          ></div>
          <div
            class="absolute inset-0 border-[3px] border-[#3d4f7c] border-t-transparent rounded-full animate-spin"
          ></div>
        </div>
      ...
      <div v-else>
          ...
          <table class="w-full">
            <thead>
              <tr class="border-b border-slate-100 bg-slate-50/60">
                ...
                <th
                  class="text-left px-6 py-3.5 text-[10px] font-bold text-slate-500 uppercase tracking-widest w-24"
                >
                  Actions
                </th>
              </tr>
              ...
              >
                <td class="px-6 py-4">
                  <span class="text-xs font-mono text-slate-400"
                    >#SR-{{ String(req.id).padStart(4, "0") }}</span
                  >
                </td>
                <td class="px-6 py-4">
                  <div class="flex items-center gap-3">
                  ...
                  >
                    <span class="relative flex h-1.5 w-1.5">
                      <span
                        ...
                        :class="statusDot(req.status)"
                      ></span>
                      <span
                        class="relative inline-flex rounded-full h-1.5 w-1.5"
                        :class="statusDotSolid(req.status)"
                      ></span>
                    </span>
        ...
        <div class="md:hidden">
          ...
          <div
            ...
            class="flex flex-col items-center gap-3 py-16 text-center px-4"
          >
            <div
              class="w-14 h-14 bg-slate-100 rounded-2xl flex items-center justify-center"
            >
          ...
          <div class="p-3 space-y-3">
            ...
            >
              ...
              <div
                class="px-4 pt-4 pb-3 flex items-start justify-between gap-3"
              >
                <div class="min-w-0">
                  <p
              ...
              <div class="grid grid-cols-2 gap-0 p-3">
                ...
                <div class="flex items-start gap-2 p-2 rounded-xl">
                  ...
                  <div class="min-w-0">
                    ...
                    <p
                      class="text-xs font-semibold text-slate-700 mt-0.5 truncate"
                    >
                      {{ req.resident_name }}
                    </p>
                  </div>
                ...
                <div class="flex items-start gap-2 p-2 rounded-xl">
                  ...
                  <div class="min-w-0">
                    ...
                    <p
                      class="text-xs font-semibold text-slate-700 mt-0.5 font-mono"
                    >
              ...
              <div
                class="mx-3 mb-3 px-3 py-2 bg-[#3d4f7c]/5 rounded-xl flex items-center justify-between border border-[#3d4f7c]/10"
              >
                <span class="text-[11px] font-semibold text-[#3d4f7c]"
      ...
      <!-- Pagination -->
      <div
        v-if="!loading && filteredRequests.length > itemsPerPage"
      ...
      <div
        v-else-if="!loading && filteredRequests.length > 0"
      ...
      >
        <p class="text-xs text-slate-400">
          ...
          result{{ filteredRequests.length !== 1 ? "s" : "" }}
        </p>
      </div>
    </div>
    ...
    <Transition name="modal">
      ...
      >
        ...
        ></div>
        <div
          class="relative bg-white w-full h-full sm:h-auto sm:rounded-2xl sm:shadow-2xl sm:max-w-4xl sm:max-h-[90vh] flex flex-col"
        >
          <!-- Modal Header -->
          <div
            class="px-4 sm:px-6 py-4 border-b border-slate-100 flex items-center justify-between flex-shrink-0"
          >
            <div class="flex items-center gap-3">
          ...
          >
            <!-- Status Banner -->
            <div
              class="flex items-center justify-between bg-slate-50 px-3 py-2.5 rounded-xl"
            >
              <span
                :class="statusBadge(selectedRequest.status)"
                class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold"
              >
                <span class="relative flex h-1.5 w-1.5">
                  <span
                    class="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75"
                    :class="statusDot(selectedRequest.status)"
                  ></span>
                  <span
                    class="relative inline-flex rounded-full h-1.5 w-1.5"
                    :class="statusDotSolid(selectedRequest.status)"
                  ></span>
                </span>
            ...
            <!-- Resident Card -->
            <div
              class="bg-[#3d4f7c] rounded-xl p-4 text-white flex items-center gap-4"
            >
              <div
                class="w-14 h-14 rounded-xl overflow-hidden border-2 border-white/30 flex-shrink-0"
              >
                <img
                ...
                <div
                  ...
                  :style="{
                    background: getAvatarColor(selectedRequest.resident_name),
                  }"
                >
              ...
              <div class="min-w-0">
                ...
                <div class="flex gap-3 mt-1.5">
                  <span
                    v-if="selectedRequest.resident_sex"
                    class="text-[10px] text-white/60"
                    >{{ selectedRequest.resident_sex }}</span
                  >
                  <span
                    v-if="calculateAge(selectedRequest.resident_birth_date)"
                    class="text-[10px] text-white/60"
                    >{{
                      calculateAge(selectedRequest.resident_birth_date)
                    }}
                    yrs</span
                  >
                  <span
                    v-if="selectedRequest.resident_civil_status"
                    class="text-[10px] text-white/60"
                    >{{ selectedRequest.resident_civil_status }}</span
                  >
                </div>
              </div>
            </div>
              ...
              <div class="bg-slate-50 rounded-xl p-3.5 space-y-2.5">
                ...
                <div class="space-y-1.5 text-xs">
                  ...
                  <div class="flex justify-between">
                    ...
                    ><span class="font-medium text-slate-700">{{
                      formatDate(selectedRequest.resident_birth_date) || "—"
                    }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">Birth Place</span
                    ><span class="font-medium text-slate-700">{{
                      selectedRequest.resident_birth_place || "—"
                    }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">Nationality</span
                    ><span class="font-medium text-slate-700">{{
                      selectedRequest.resident_nationality || "Filipino"
                    }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">Occupation</span
                    ><span class="font-medium text-slate-700">{{
                      selectedRequest.resident_occupation || "—"
                    }}</span>
                  </div>
                </div>
              </div>
              <div class="bg-slate-50 rounded-xl p-3.5 space-y-2.5">
                ...
                <div class="space-y-1.5 text-xs">
                  <div class="flex justify-between">
                    ...
                    ><span class="font-medium text-slate-700">{{
                      selectedRequest.resident_house_no || "—"
                    }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">Purok</span
                    ><span class="font-medium text-slate-700">{{
                      selectedRequest.resident_purok || "—"
                    }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">Sitio</span
                    ><span class="font-medium text-slate-700">{{
                      selectedRequest.resident_sitio || "—"
                    }}</span>
                  </div>
                </div>
              </div>
              <div class="bg-slate-50 rounded-xl p-3.5 space-y-2.5">
                ...
                <div class="space-y-1.5 text-xs">
                  <div class="flex justify-between">
                    ...
                    ><span
                      class="font-medium text-slate-700 text-right max-w-[120px]"
                      >{{ formatType(selectedRequest.type) }}</span
                    >
                  </div>
                  <div class="flex justify-between">
                ...
                <div v-if="selectedRequest.purpose" class="pt-1">
                  ...
                  <p
                    class="text-xs text-slate-700 bg-white p-2 rounded-lg border border-slate-100"
                  >
            ...
            <div
              v-if="selectedRequest.remarks"
              class="bg-purple-50 border border-purple-100 rounded-xl p-3.5"
            >
              <p
                class="text-[10px] font-bold text-purple-600 uppercase tracking-wider mb-1.5"
              >
                Official Remarks
              </p>
            ...
            >
              ...
              <div v-if="showRejectInput" class="mt-3 space-y-2">
                <textarea
                  ...
                  class="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#3d4f7c]/50 bg-slate-50 focus:bg-white resize-none"
                ></textarea>
                <div class="flex justify-end gap-2">
                  <button
                    @click="
                      ...
                      rejectionRemarks = '';
                    "
                    class="px-3 py-1.5 text-xs font-semibold text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
                    @click="confirmReject(selectedRequest)"
                    class="px-3 py-1.5 text-xs font-semibold text-white bg-red-500 rounded-lg hover:bg-red-600 cursor-pointer"
                  >
                    Confirm Disapprove
                  </button>
                </div>
            ...
            <div
              v-else-if="selectedRequest.status === 'completed'"
              class="bg-emerald-50 border border-emerald-200 rounded-xl p-3.5 text-center"
            >
            ...
            <div
              v-else-if="selectedRequest.status === 'cancelled'"
              class="bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-center"
            >
            ...
            <div
              v-else-if="selectedRequest.status === 'rejected'"
              class="bg-red-50 border border-red-200 rounded-xl p-3.5 text-center"
            >
...
const filteredRequests = computed(() => {
  ...
  if (filters.search) {
    ...
    r = r.filter(
      (x) =>
        x.resident_name?.toLowerCase().includes(s) ||
...
const paginatedRequests = computed(() => {
  ...
  return filteredRequests.value.slice(s, s + itemsPerPage);
});

async function fetchRequests() {
  loading.value = true;
  try {
    const data = await ServiceRequestService.getPending();
...
async function confirmApprove(req) {
  const result = await Swal.fire({
    ...
    text: `Approve "${formatType(req.type)}" for ${req.resident_name}?`,
    icon: "question",
...
async function confirmReject(req) {
  const result = await Swal.fire({
    ...
    text: `Disapprove "${formatType(req.type)}" for ${req.resident_name}?`,
    icon: "warning",
    showCancelButton: true,
  ...
  if (!result.isConfirmed) return;
  try {
    await ServiceRequestService.reject(req.id, {
