      ...
      <div class="px-6 py-5 max-h-[60vh] overflow-y-auto scrollbar-thin">
        <form @submit.prevent="submit" class="space-y-5">
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <!-- Image preview area - compact 2x2 size -->
            <div class="w-full border-2 border-dashed border-[#3d4f7c]/30 rounded-xl p-3 bg-[#3d4f7c]/5 flex items-center gap-3 transition-all hover:border-[#3d4f7c]/50 hover:bg-[#3d4f7c]/10">
              ...
              <template v-if="imagePreview">
                <div class="relative flex-shrink-0">
                  ...
                  <button type="button" class="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center hover:bg-red-600 transition-all shadow-sm" @click.stop="removeImage">
                ...
                <div class="flex-1 min-w-0">
                  <p class="text-xs font-medium text-slate-700 truncate">{{ imageFile?.name || 'Image uploaded' }}</p>
              ...
              <template v-else>
                ...
                <div class="flex flex-col gap-1.5">
                  <button 
                    ...
                    class="inline-flex items-center justify-center gap-1 px-2 py-1.5 bg-[#3d4f7c] text-white text-[10px] font-semibold rounded-lg hover:bg-[#252b3b] transition-all whitespace-nowrap"
                  >
                  ...
                  <button 
                    type="button"
                    @click="openCamera"
                    class="inline-flex items-center justify-center gap-1 px-2 py-1.5 bg-emerald-600 text-white text-[10px] font-semibold rounded-lg hover:bg-emerald-700 transition-all whitespace-nowrap"
                  >
                    <svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <!-- Proof of Identity Note - compact version -->
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <input 
              ...
              :min="minDate"
              @input="validateDate"
              @keydown="preventWeekendSelection"
            ...
            <span class="text-[10px] text-slate-400">Select your preferred date for processing (Weekdays only)</span>
          </div>
...
const validateDate = (event) => {
  ...
  if (day === 0 || day === 6) {
    errors.value.preferred_date = 'Weekends (Saturday & Sunday) are not available';
...
// Prevent typing/selecting weekend dates manually
const preventWeekendSelection = (event) => {
  // Allow backspace, delete, tab, escape, enter
  if ([8, 9, 13, 27, 46].includes(event.keyCode)) return;
...
const validateForm = () => {
  ...
  if (!formData.value.preferred_date) {
    ...
    const selectedDate = new Date(formData.value.preferred_date);
    const day = selectedDate.getDay();
    if (day === 0 || day === 6) {
      newErrors.preferred_date = 'Weekends (Saturday & Sunday) are not available';
    }
  }
...
/* Style for disabled weekend dates in some browsers */
input[type="date"]::-webkit-datetime-edit-day-field[value="0"],
input[type="date"]::-webkit-datetime-edit-day-field[value="6"] {
