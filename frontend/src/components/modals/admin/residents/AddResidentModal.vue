<template>
  ...
  >
    <div class="bg-white rounded-2xl w-full max-w-5xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">

      <!-- Header -->
      <div class="px-6 pt-5 pb-0 flex-shrink-0 bg-white">
        <div class="flex items-center justify-between mb-5">
          <div>
            <h2 class="text-base font-bold text-slate-800">Add New Resident</h2>
            <p class="text-xs text-slate-400 mt-0.5">Step {{ currentStep }} of {{ steps.length }} — {{ steps[currentStep - 1].label }}</p>
          </div>
          <button
          ...
          <template v-for="(step, i) in steps" :key="step.id">
            ...
            <div class="flex flex-col items-center flex-1">
              <div class="flex items-center w-full">
                ...
                <!-- Circle -->
                <div
                  class="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 border-2 text-xs font-bold"
                  :class="
                    i + 1 < currentStep
                ...
                >
                  ...
                  <svg v-if="i + 1 < currentStep" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/>
                  </svg>
                  <!-- Icon for current/future -->
                  <component :is="'svg'" v-else width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
              ...
              <!-- Label -->
              <span
                class="text-[10px] font-semibold mt-1.5 transition-all duration-200 text-center leading-tight"
      ...
      <!-- Body -->
      <div class="px-6 py-5 overflow-y-auto flex-1 scrollbar-thin">
        ...
        <div v-if="currentStep === 1" class="space-y-4">
          <div class="flex items-center gap-2 mb-1">
            ...
            <h3 class="text-sm font-bold text-slate-700">Profile Image</h3>
          </div>

          <div class="flex flex-col gap-3">
            <!-- Image preview area -->
            <div class="w-full border-2 border-dashed border-[#3d4f7c]/30 rounded-xl p-6 bg-[#3d4f7c]/5 flex flex-col items-center justify-center text-center transition-all hover:border-[#3d4f7c]/50 hover:bg-[#3d4f7c]/10 cursor-pointer" @click="triggerFileInput">
              <template v-if="imagePreview">
                <div class="relative w-full max-w-xs mx-auto">
                  <img :src="imagePreview" class="w-full h-auto rounded-lg shadow-md" alt="Preview" />
                  <button class="absolute -top-2 -right-2 w-8 h-8 bg-red-500 text-white rounded-full flex items-center justify-center hover:bg-red-600 transition-all shadow-md" @click.stop="removeImage">
              ...
              <template v-else>
                ...
                <p class="text-xs text-slate-400 mb-3">JPG, PNG or GIF (Max 4MB)</p>
                <div class="inline-flex items-center gap-2 px-4 py-2 bg-[#3d4f7c] text-white text-sm font-semibold rounded-xl hover:bg-[#252b3b] transition-all cursor-pointer">
        ...
        <div v-if="currentStep === 2" class="space-y-4">
          <div class="flex items-center gap-2 mb-1">
            ...
            <h3 class="text-sm font-bold text-slate-700">Personal Information</h3>
          </div>
          ...
          <div class="grid grid-cols-4 gap-3">
            <div class="flex flex-col gap-1.5">
              ...
              <span v-if="errors.first_name" class="text-[10px] text-red-400">{{ errors.first_name }}</span>
            </div>
            <div class="flex flex-col gap-1.5">
              ...
              <input v-model="form.middle_name" type="text" placeholder="Middle name"
                class="border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-[#3d4f7c]/50 transition-all bg-slate-50 focus:bg-white placeholder-slate-300"/>
            </div>
            <div class="flex flex-col gap-1.5">
              ...
              <span v-if="errors.last_name" class="text-[10px] text-red-400">{{ errors.last_name }}</span>
            </div>
            <div class="flex flex-col gap-1.5">
          ...
          <div class="grid grid-cols-3 gap-3">
            <div class="flex flex-col gap-1.5">
              ...
              <span v-if="errors.email" class="text-[10px] text-red-400">{{ errors.email }}</span>
            </div>
            <div class="flex flex-col gap-1.5">
              ...
              <span v-if="errors.contact_number" class="text-[10px] text-red-400">{{ errors.contact_number }}</span>
            </div>
            <div class="flex flex-col gap-1.5">
              ...
              <span v-if="errors.sex" class="text-[10px] text-red-400">{{ errors.sex }}</span>
            </div>
          </div>
          ...
          <div class="grid grid-cols-3 gap-3">
            <div class="flex flex-col gap-1.5">
              ...
              <span v-if="errors.birth_place" class="text-[10px] text-red-400">{{ errors.birth_place }}</span>
            </div>
            <div class="flex flex-col gap-1.5">
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <span v-if="errors.civil_status" class="text-[10px] text-red-400">{{ errors.civil_status }}</span>
            </div>
          </div>
          ...
          <div class="grid grid-cols-5 gap-3">
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <span v-if="errors.nationality" class="text-[10px] text-red-400">{{ errors.nationality }}</span>
            </div>
            <div class="flex flex-col gap-1.5">
        ...
        <div v-if="currentStep === 3" class="space-y-4">
          <div class="flex items-center gap-2 mb-1">
            ...
            <h3 class="text-sm font-bold text-slate-700">Emergency Contact</h3>
          </div>
          ...
          <div class="grid grid-cols-3 gap-3">
            <div class="flex flex-col gap-1.5">
              ...
              <span v-if="errors.emergency_contact_person" class="text-[10px] text-red-400">{{ errors.emergency_contact_person }}</span>
            </div>
            <div class="flex flex-col gap-1.5">
              <label class="text-xs font-semibold text-slate-600">Contact Number <span class="text-red-400">*</span></label>
              ...
              <span v-if="errors.emergency_contact_number" class="text-[10px] text-red-400">{{ errors.emergency_contact_number }}</span>
            </div>
            <div class="flex flex-col gap-1.5">
              ...
              <span v-if="errors.emergency_contact_relationship" class="text-[10px] text-red-400">{{ errors.emergency_contact_relationship }}</span>
            </div>
          </div>
        </div>
        ...
        <div v-if="currentStep === 4" class="space-y-4">
          ...
          <div class="grid grid-cols-3 gap-3">
            <div class="flex flex-col gap-1.5">
              ...
              <span v-if="errors.house_no" class="text-[10px] text-red-400">{{ errors.house_no }}</span>
            </div>
            <div class="flex flex-col gap-1.5">
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <span v-if="errors.sitio" class="text-[10px] text-red-400">{{ errors.sitio }}</span>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="flex flex-col gap-1.5">
              ...
              <span v-if="errors.latitude" class="text-[10px] text-red-400">{{ errors.latitude }}</span>
            </div>
            <div class="flex flex-col gap-1.5">
              ...
              <span v-if="errors.longitude" class="text-[10px] text-red-400">{{ errors.longitude }}</span>
            </div>
          </div>
        </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <input type="text" value="adminadmin" disabled
                class="border border-slate-200 bg-slate-100 text-slate-400 rounded-xl px-3 py-2 text-sm cursor-not-allowed"/>
              <span class="text-[10px] text-slate-400">Temporary default password</span>
            </div>
          </div>
          ...
          <div class="mt-2 p-4 bg-[#3d4f7c]/5 border border-[#3d4f7c]/15 rounded-xl">
            ...
            <div class="grid grid-cols-2 gap-x-6 gap-y-1.5 text-xs">
              <div class="flex justify-between">
                ...
                <span class="font-semibold text-slate-700">{{ [form.first_name, form.middle_name, form.last_name, form.suffix].filter(Boolean).join(' ') || '—' }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-slate-400">Email</span>
                <span class="font-semibold text-slate-700 truncate max-w-[140px]">{{ form.email || '—' }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-slate-400">Contact</span>
                <span class="font-semibold text-slate-700">{{ form.contact_number || '—' }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-slate-400">Sex</span>
                <span class="font-semibold text-slate-700">{{ form.sex || '—' }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-slate-400">Emergency Contact</span>
                <span class="font-semibold text-slate-700">{{ form.emergency_contact_person || '—' }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-slate-400">Address</span>
                <span class="font-semibold text-slate-700">{{ form.house_no ? `${form.house_no}, ${form.sitio}` : '—' }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- Footer -->
      <div class="flex items-center justify-between px-6 py-4 border-t border-slate-100 bg-slate-50/60 flex-shrink-0">
        <button
          v-if="currentStep > 1"
          class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 hover:border-slate-300 transition-all cursor-pointer active:scale-95"
          @click="currentStep--"
        >
          <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
        ...
        <button v-else class="invisible px-4 py-2.5">placeholder</button>

        <div class="flex items-center gap-2">
          <button
            class="px-4 py-2.5 text-sm font-semibold text-slate-500 hover:text-slate-700 transition-all cursor-pointer border-0 bg-transparent"
            @click="$emit('close')"
          >Cancel</button>
...
function nextStep() {
  if (isStepValid(currentStep.value)) currentStep.value++;
}
...
function formatPhoneNumber(field, event) {
  ...
  if (value.length >= 5)      value = value.slice(0, 4) + ' ' + value.slice(4, 7) + ' ' + value.slice(7, 11);
...
async function submit() {
  ...
  if (!f.contact_number)                       newErrors.contact_number = 'Required.';
  ...
  if (!f.emergency_contact_number)         newErrors.emergency_contact_number = 'Required.';
  else if (!isValidPhoneNumber(f.emergency_contact_number)) newErrors.emergency_contact_number = 'Must be 11 digits starting with 0.';
  if (!f.emergency_contact_relationship)   newErrors.emergency_contact_relationship = 'Required.';
  if (!f.house_no)  newErrors.house_no  = 'Required.';
  ...
  if (Object.keys(newErrors).length > 0) {
    await Swal.fire({ icon: 'error', title: 'Validation Error', text: 'Please fill in all required fields.', confirmButtonColor: '#3d4f7c' });
    return;
  }

  loading.value = true;
  Swal.fire({ title: 'Please wait...', text: 'Creating resident account...', allowOutsideClick: false, allowEscapeKey: false, didOpen: () => Swal.showLoading() });
  ...
  try {
    ...
    Swal.close();
    await Swal.fire({ icon: 'success', title: 'Success!', text: 'Resident has been created successfully.', timer: 2000, showConfirmButton: false });
    emit('saved');
    emit('close');
  } catch (err) {
    Swal.close();
    if (err.response?.data?.errors) {
      errors.value = Object.fromEntries(Object.entries(err.response.data.errors).map(([k, v]) => [k, v[0]]));
      Swal.fire({ icon: 'error', title: 'Validation Error', text: 'Please check the form for errors.', confirmButtonColor: '#3d4f7c' });
    } else {
      Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to create resident. Please try again.', confirmButtonColor: '#3d4f7c' });
    }
  } finally {
    loading.value = false;
  }
