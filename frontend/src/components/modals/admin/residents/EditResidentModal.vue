<template>
  ...
  >
    ...
    >
      ...
      >
        <h2 class="text-base font-semibold text-gray-900">Edit Resident</h2>
        <button
  ...
  (resident) => {
    if (resident) {
      ...
      form.value = {
        ...
        emergency_contact_number: info.emergency_contact_number ?? "",
        emergency_contact_relationship:
        ...
        status: resident.status?.status ?? 1,
      };
    }
  },
  { immediate: true },
...
async function submit() {
  ...
  loading.value = true;
  try {
    await ResidentService.updateResident(props.resident.id, form.value);
    emit("saved");
    emit("close");
  } catch (err) {
...
async function handleResetPassword() {
  if (!confirm("Reset this resident's password to default (adminadmin)?"))
    return;
