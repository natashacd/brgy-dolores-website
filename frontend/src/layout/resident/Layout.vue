<template>
  <div class="h-screen bg-gray-100 flex overflow-hidden">
            ...
            <div class="flex-shrink-0">
              <div class="w-12 h-12 rounded-full flex items-center justify-center" :class="notificationIconBg">
                <svg class="w-6 h-6" :class="notificationIconColor" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path v-if="notificationType === 'approved'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  <path v-else-if="notificationType === 'processing'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                  <path v-else-if="notificationType === 'completed'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  <path v-else-if="notificationType === 'rejected'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      <!-- Top Header with Mobile Menu Button and Notification Bell - Larger Font -->
      <header class="bg-white shadow-sm h-20 flex-shrink-0 flex items-center px-6">
        ...
        <!-- Desktop Title and Notifications - Larger Font -->
...
const maxDemoNotifications = 5 
let demoNotificationCount = 0
...
const saveNotifications = () => {
  localStorage.setItem('resident_notifications', JSON.stringify(notifications.value))
}

const loadNotifications = () => {
  const saved = localStorage.getItem('resident_notifications')
  if (saved) {
    try {
...
const triggerSampleNotifications = () => {
  const sampleNotifs = [
    {
      ...
      message: 'Your BRGY ID Request has been approved by the admin.',
      type: 'approved'
    },
    {
      title: 'Request Rejected',
      message: 'Your BRGY Business Clearance was rejected. Reason: Incomplete requirements.',
      type: 'rejected'
    },
    {
      title: 'Request Completed',
      message: 'Your BRGY Certificate of Indigency is ready for pickup.',
      type: 'completed'
    },
    {
      title: 'Request Processing',
      message: 'Your BRGY Certificate of Residency is now being processed.',
      type: 'processing'
    },
    {
      title: 'Status Updated',
      message: 'Your request status has been updated to Processing.',
      type: 'processing'
    }
  ]
  ...
  notificationInterval.value = setInterval(() => {
    if (count < 3) { 
      const randomIndex = Math.floor(Math.random() * sampleNotifs.length)
      const notif = sampleNotifs[randomIndex]
      addNotification(notif.title, notif.message, notif.type)
      count++
    } else {
      clearInterval(notificationInterval.value)
      notificationInterval.value = null
      console.log('Demo notifications stopped after 3 messages')
    }
  }, 8000) 
}
...
onMounted(() => {
  ...
  setTimeout(() => {
    addNotification(
      ...
      'You will receive notifications when your requests are updated.',
      'info'
    )
  }, 2000)
...
.bg-emerald-500, .bg-blue-500, .bg-green-500, .bg-red-500 {
